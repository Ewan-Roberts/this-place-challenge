 ...
 /* ************************************************************************
  * Responses can take anywhere up to 1.5s to return. Returns a promise    *
  * that returns an array of region objects that contain a parent region   *
  * ("" when at the top level), creating a hierarchical tree of data. This *
...
function getPropertiesByRegion(regions) {
  ...
  return new Promise((resolve) => {
    ...
    for( element in array_of_regions ) {
      const desired_region = array_of_regions[element].trim();
    ...
    setTimeout(() => resolve(properties_to_return), Math.random() * 1000 * number_of_regions_requested);
  });
}
...
function getInvestableRegions() {
  return new Promise((resolve) => {
    const investable_regions = {
      regions: [ "camden", "kew", "rochdale" ],
    }

    setTimeout(() => resolve(investable_regions), Math.random() * 3000);
  });
}
...
function displayResults(data) {
  ...
  * should run on Node v6.10.3 (the latest available for AWS Lambda).      *
  * ********************************************************************** */




const match = (list, str) => {
    ...
    for (let i = 0; i < list.length; i++) {
        ...
        if(bool){children.push(list[i].name)}
    }

    return children;
}

const trimTree = (data,startingPoint) =>{

    data = data.regions
    
    let tree = [{name:startingPoint,level:0}]

    const rankList = (str,num) => {

        for (var i = 0; i < data.length; i++) {

            if(data[i].parent === str){

                tree.push({name:data[i].name,level:num+1})
                
                rankList(data[i].name,num+1)

            }
        }
    }

    rankList(startingPoint,0)

    return tree 
}

const consolidate = (data,regions) =>{

    let mergedRegions= [];

    for (var i = 0; i < regions.length; i++) {
        
        let regionMatches = match(data,regions[i])
        mergedRegions.push.apply(mergedRegions, regionMatches);

    }

    const removeDups = Array.from(new Set(mergedRegions))

    return removeDups;

}

console.time('getInvestableProperties');
console.time('cachedProperty');

function getInvestableProperties(top_level_region) {

    return new Promise((resolve)=>{

        Promise.all([getAllRegions(), getInvestableRegions()]).then(locationData =>{
            
            if(locationData[1].regions.length <= 0){
                throw "Nothing entered into: getInvestableRegions()"
            }

            const trimmedTree = trimTree(locationData[0],top_level_region)
            const finalResult = consolidate(trimmedTree,locationData[1].regions)

            return finalResult

        }).then(investmentRegion => {

            let requestsArray = []

            for (var i = 0; i < investmentRegion.length; i++) {
                
                requestsArray.push(getPropertiesByRegion(investmentRegion[i]))

            }

            Promise.all(requestsArray).then(investmentProperties => { 
                
                let requestResult = [];
                
                for (var i = 0; i < investmentProperties.length; i++) {

                    if ((investmentProperties[i].properties).length > 0){

                        requestResult.push(investmentProperties[i].properties[0])

                    }
                }

                resolve({properties:requestResult})

            });

            console.timeEnd('getInvestableProperties');
